...
Веб‑приложение на Gradio, которое общается с локальной моделью LM Studio через
OpenAI‑совместимый API. Поддерживается отправка текста и изображений в одном
сообщении. Все диалоги сохраняются между запусками, можно создавать несколько
чатов.
...
2. Создайте файл `.env` (пример ниже) или экспортируйте переменные окружения:
   ...
   - `LMSTUDIO_URL` — адрес LM Studio (`http://172.23.32.1:1234` по умолчанию)
